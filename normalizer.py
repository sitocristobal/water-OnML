...
and normalizer() takes the filled data base and adds the remaing days to set every month to 31 days.
...
def normalizer(File, timeframe, timestep):
    ...
    if timestep == '15 min':
        ...
        # Save temp file
        df.index.name = 'date'
        df.to_csv(f'data/{fileName}_temp.csv', sep=';', encoding='utf-8', index=True, header=['value', 'year', 'month', 'day', 'hour', 'minute', 'second'])
        ...
        # Save temp file
        df.index.name = 'date'
        df.to_csv(f'data/{fileName}_temp.csv', sep=';', encoding='utf-8', index=True, header=['value', 'year', 'month', 'day'])
